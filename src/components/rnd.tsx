...
/**
 * Imported from https://github.com/bokuweb/react-rnd
 */
export class Rnd extends React.PureComponent<Props, State> {
  ...
  getMaxSizesFromProps(): MaxSize {
    const maxWidth =
      typeof this.props.maxWidth === 'undefined' ? Number.MAX_SAFE_INTEGER : this.props.maxWidth;
    const maxHeight =
      typeof this.props.maxHeight === 'undefined' ? Number.MAX_SAFE_INTEGER : this.props.maxHeight;
  ...
  onDragStart(e: RndDragEvent, data: DraggableData) {
    ...
    if (this.props.bounds === 'parent') {
      ...
      const bottom =
        (document.body.offsetHeight - this.resizable.size.height * scale) / scale + top;
  ...
  onResizeStart(
    ...
    if (this.props.bounds) {
      ...
      if (
        ...
        const boundaryRect =
          this.props.bounds === 'window' ? { left: 0, top: 0 } : boundary.getBoundingClientRect();
        const boundaryLeft = boundaryRect.left;
        const boundaryTop = boundaryRect.top;
  ...
  render() {
    ...
    return (
      <Draggable
        ...
        // @ts-expect-error `onMouseUp` is not a prop of `Draggable`
        onMouseUp={onMouseUp}
